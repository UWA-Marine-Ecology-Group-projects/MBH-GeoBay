...
zones <- readRDS( "GBZones_forDesign3.RDS") # this one in different folder
...
straw.nums <- readRDS( "StrawmanNumbers_Zones.RDS") # this one in different folder
...
plot(inclProbs)
points(newSites, pch=20, col='black', add=T)

pdf( "~/MBHdesignGB/Design3/CentreOfClusters_design3.pdf", height=7, width=8)
#plot( inclProbs, main="Clusters")
plot( gb_rasters$bathy, main = "Clusters")
points(newSites, add=T, pch=20, col = 'black', add = TRUE)
plot( gb, add=TRUE)
dev.off()
...
getlocal <- function(ii){
  ...
  pbuf <- buffer( r2, width=4000) ## units are in metres
...
fullSamplep$cluster <- rep( do.call( "c", fullZones), each=45) # 15 is number of point per transect
#tpoints <- fullSample$points[,c("x","y")]
write.csv(fullSamplep, "~/MBHdesignGB/outputs/GB_design1_Transects.csv")
...
writeOGR( fullSamplesp, dsn="~/MBHdesignGB/outputs/", layer=paste( "newSites", Sys.Date(), sep="_"), driver="ESRI Shapefile", overwrite_layer=TRUE)
