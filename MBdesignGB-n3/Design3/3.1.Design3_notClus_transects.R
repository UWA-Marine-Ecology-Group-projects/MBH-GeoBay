...
plot(inclProbs)
inclProbs <- raster( x="inclProbs_design3.tif")
...
zones <- readRDS( "GBZones_forDesign1.RDS")
...
newSitesT <- list(NPZ=NULL,HPZ=NULL,SPZ=NULL,MUZ=NULL)
for( zz in c("NPZ", "HPZ", "SPZ", "MUZ")){
  print( zz)
...
newSitesTpoints1 <- rbind (newSitesT$NPZ$points, newSitesT$HPZ$points) 
newSitesTpoints <- rbind (newSitesT$NPZ$points, newSitesT$HPZ$points, newSitesT$SPZ$points, newSitesT$MUZ$points)

newSitesTsp1 <- newSitesTpoints1
coordinates(newSitesTsp1) <- ~x+y
#some of the spatial balance is not great...  Presumably because the balance of the reference sites is also not great...

plot(gb)
points(newSitesTsp1, add=T)

fzones <- list("NPZ1", "NPZ2", "NPZ3", 'NPZ4', 'NPZ5', 'NPZ6', 'NPZ7', 'NPZ8', 'NPZ9', "HPZ1", "HPZ2", "HPZ3", "HPZ4", "HPZ5", 'HPZ6')
               #"SPZ1", "SPZ2", "SPZ3", "SPZ4", "SPZ5", "SPZ6", "MUZ1", "MUZ2", "MUZ3", "MUZ4", "MUZ5", "MUZ6")
...
newSitesTpoints1$cluster <- rep( do.call( "c", fzones), each=25) # 15 is number of point per transect
#tpoints <- fullSample$points[,c("x","y")]
write.csv(newSitesTpoints1, "~/MBHdesignGB/Design3/GB_design3NPZ_HPZ_TransectsNotClus.csv")

newSitesTsp1 <- SpatialPointsDataFrame( coords=newSitesTpoints1[,c("x","y")], data=newSitesTpoints1, proj4string=CRS(proj4string(inclProbs)))
...
writeOGR( newSitesTsp1, dsn="~/MBHdesignGB/outputs/newdesign", layer=paste( "Design3_NPZ_HPZ_notClustTransects", Sys.Date(), sep="_"), driver="ESRI Shapefile", overwrite_layer=TRUE)
...
newSitesTsp <- newSitesT$SPZ$points
newSites <- newSitesT$SPZ$points

#newSitesTpoints <- rbind (newSitesT$SPZ$points)
...
plot(gb, add=T)
plot(gb_rasters$bathy)
points(newSitesTsp, add=T)

fzones <- list("SPZ1", "SPZ2", "SPZ3", "SPZ4", "SPZ5", "SPZ6",
               'SPZ7', 'SPZ8', 'SPZ9', 'SPZ10', 'SPZ11', 'SPZ12')
#"SPZ1", "SPZ2", "SPZ3", "SPZ4", "SPZ5", "SPZ6", "MUZ1", "MUZ2", "MUZ3", "MUZ4", "MUZ5", "MUZ6")
#fullSamplep <- do.call( "rbind", fullSample[19:36])
#fullSamplet <- do.call( "rbind", fullSample$transects)
#fullSample2 <- bind_rows(fullSample, .id = "column_label")
newSites$cluster <- rep( do.call( "c", fzones), each=25) # 15 is number of point per transect
head(newSites)
#tpoints <- fullSample$points[,c("x","y")]
write.csv(newSites, "~/MBHdesignGB/Design3/GB_design3SPZ_TransectsNotClus.csv")
...
writeOGR( newSitesTsp, dsn="~/MBHdesignGB/Design3", layer=paste( "Design3_SPZnotClustTransects", Sys.Date(), sep="_"), driver="ESRI Shapefile", overwrite_layer=TRUE)
#writeOGR( legacySites, dsn="C:/Users/21933549/Dropbox/UWA/Research Associate/Ningaloo19_Data", layer=paste( "legacySites", Sys.Date(), sep="_"), driver="ESRI Shapefile", overwrite_layer=TRUE)
proj4string(newSitesTsp) <- proj4string(gb_rasters$bathy)
pspz <- spTransform(newSitesTsp, "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")
writeOGR( pspz, dsn="~/MBHdesignGB/Design3", layer=paste( "coords_spz", Sys.Date(), sep="_"), driver="ESRI Shapefile", overwrite_layer=TRUE)

newSitesT <- list(MUZ=NULL)
for( zz in c("MUZ")){
  print( zz)
  #the number of samples to take (specified minus the legacy number)
  numby <- floor( (straw.nums[zz]))
...
newSites <- newSitesT$MUZ$points

newSitesTsp <- newSites
coordinates(newSitesTsp) <- ~x+y
#some of the spatial balance is not great...  Presumably because the balance of the reference sites is also not great...
...
fzones <- list("MUZ1", "MUZ2", "MUZ3", "MUZ4", "MUZ5", "MUZ6", 'MUZ7', 'MUZ8', 'MUZ9', 'MUZ10', 
               'MUZ11', 'MUZ12','MUZ13','MUZ14','MUZ15','MUZ16')
#"SPZ1", "SPZ2", "SPZ3", "SPZ4", "SPZ5", "SPZ6", "MUZ1", "MUZ2", "MUZ3", "MUZ4", "MUZ5", "MUZ6")
#fullSamplep <- do.call( "rbind", fullSample[19:36])
#fullSamplet <- do.call( "rbind", fullSample$transects)
#fullSample2 <- bind_rows(fullSample, .id = "column_label")
newSites$cluster <- rep( do.call( "c", fzones), each=25) # 15 is number of point per transect
#tpoints <- fullSample$points[,c("x","y")]
write.csv(newSitesTpoints, "~/MBHdesignGB/Design3/GB_design3MUZ_TransectsNotClus.csv")
...
writeOGR( newSitesTsp, dsn="~/MBHdesignGB/Design3", layer=paste( "Design3_MUZnotClustTransects", Sys.Date(), sep="_"), driver="ESRI Shapefile", overwrite_layer=TRUE)
#writeOGR( legacySites, dsn="C:/Users/21933549/Dropbox/UWA/Research Associate/Ningaloo19_Data", layer=paste( "legacySites", Sys.Date(), sep="_"), driver="ESRI Shapefile", overwrite_layer=TRUE)
proj4string(newSitesTsp) <- proj4string(gb_rasters$bathy)

pmuz <- spTransform(newSitesTsp, "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")
pmuz
writeOGR(pmuz, dsn="~/MBHdesignGB/Design3", layer=paste( "coords_MUZ", Sys.Date(), sep="_"), driver="ESRI Shapefile", overwrite_layer=TRUE)

bathy
dsp <- d
coordinates(dsp) <- ~x+y
plot(dsp)
proj4string(dsp) <- proj4string(bathy)

s <- readOGR("~/MBHdesignGB/outputs/newdesign/Design3_SPZnotClustTransects.shp")
m <- readOGR("~/MBHdesignGB/outputs/newdesign/Design3_MUZnotClustTransects.shp")
nh <- readOGR("~/MBHdesignGB/outputs/newdesign/Design3_NPZ_HPZ_notClustTransects.shp")
### Change to lat long
plot(gb)
plot(s, add=T)
plot(m, add=T)
...
b2 <- proj4string(b)
dsp2 <- spTransform(dsp, b2)

dsp2 <- as.data.frame(dsp2)
head(dsp2)
##################################
...
newSitesLocations_dms <- apply( coordinates(d), 2, convert2dms)
...
write.csv(newsitesLatLon3, paste(o2.dir, "Design2_notClusTrans_LatLon.csv", sep='/'))
