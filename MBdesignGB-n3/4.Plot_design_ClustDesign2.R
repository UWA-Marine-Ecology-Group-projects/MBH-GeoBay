...
pdf( "~/MBHdesignGB/Design3/SPZ.pdf")
#not for the Design in each zone
par( mfrow=c(1,1), mar=c(1,1,1,1), oma=c(0,0,2,2))
...
tmp <- crop( tmp, zoney)
plot( tmp, add=TRUE)
spzsites <- raster::intersect(spz, zoney)
...
tmp <- crop( tmp, zoney)
plot( tmp, add=TRUE)
hpzsites <- raster::intersect(fullSamplesp, zoney)
...
#plot( newSites[as.vector( !is.na( zoneID)),], add=TRUE, col='black', pch=20)
#legend( "topleft", legend=c("Legacy Sites","New Sites"), bty='n', col=c("red","blue"), pch=c(4,20))
npzsites <- raster::intersect(fullSamplesp, zoney)
