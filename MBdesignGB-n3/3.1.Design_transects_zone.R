...
newSitesTpoints$cluster <- rep( do.call( "c", fullZones), each=25) # 15 is number of point per transect
#tpoints <- fullSample$points[,c("x","y")]
write.csv(newSitesTpoints, "~/MBHdesignGB/outputs/GB_design2_TransectsNotClus.csv")
...
writeOGR( newSitesTsp, dsn="~/MBHdesignGB/outputs/", layer=paste( "Design2_Transects", Sys.Date(), sep="_"), driver="ESRI Shapefile", overwrite_layer=TRUE)
