...
clusterpoints <- quasiSamp( n=6, potential.sites=coordinates( bathy), inclusion.probs=values(rasterIP), nSampsToConsider=10000)
cp <- clusterpoints
coordinates(cp) <- ~x+y
...
#### transect properties and constrains ####
...
table( as.vector(gb.constrains)) #    289234           107425              228             3458 
...
#tmpMat <- matrix( apply( gb.constraints.bool, 1, sum), nrow=nrow( pot.sites), ncol=ncol( pot.sites))
tmpMat <- matrix( apply( gb.constraints.bool, 1, sum), nrow=n.y, ncol=n.x)
#tmpMat2 <- t(tmpMat)

image.plot( y=1:n.x, x=1:n.y, z=tmpMat,
...
for( ii in 1:nrow( cp)){
  tmp <- getlocal(ii)
  fullZones[[ii]] <- rownames( cp@data)[ii]
  tmpm <- raster::as.matrix(rasterIP)
  tmpm <- t(tmpm)
  tmpdf <- as.data.frame (
