...
inclProbs <- raster( x="inclProbs_design1.tif")
...
fzones <- list("NPZ1", "NPZ2", "NPZ3", "HPZ1", "HPZ2", "HPZ3", "SPZ1", "SPZ2", "SPZ3", "SPZ4", "SPZ5", "SPZ6", "MUZ1", "MUZ2", "MUZ3", "MUZ4", "MUZ5", "MUZ6")
...
newSitesTpoints$cluster <- rep( do.call( "c", fzones), each=5) # 15 is number of point per transect
#tpoints <- fullSample$points[,c("x","y")]
write.csv(newSitesTpoints, "~/MBHdesignGB/outputs/newdesign/GB_design2_TransectsNotClus.csv")
...
writeOGR( newSitesTsp, dsn="~/MBHdesignGB/outputs/newdesign", layer=paste( "Design2_notClustTransects", Sys.Date(), sep="_"), driver="ESRI Shapefile", overwrite_layer=TRUE)
