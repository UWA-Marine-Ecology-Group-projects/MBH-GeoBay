...
GBDat <- readRDS( "GBData_forDesign1.RDS")
gb_rasters <- readRDS("GBRasters_forDesign1.RDS")
...
straw.nums <- c( 6, 6, 3, 3)  #numbers of drops
...
Bathy.quant <- c(0,0.5,0.75,0.9,1)
...
pdf( "~/MBHdesignGB/outputs/newdesign/InclusionProbs.pdf", height=7, width=8)
plot( catB, main="Inclusion Probabilities for Zones Design") # -46.900249 -30.000000 -23.105238 -17.000000  -8.540641 
plot( gb, add=TRUE)
dev.off()
...
inclProbs@data@values[inclProbs@data@values %in% c(0,1,2,3,4)] <- NA  #cheats way to crop
plot( inclProbs)
...
writeRaster( inclProbs, file='inclProbs_clus_design2.tif', overwrite=TRUE)

pdf( "~/MBHdesignGB/outputs/newdesign/inclProbs_clus_design2.pdf", height=7, width=8)
plot( inclProbs, main="Clustered transects design 250m (n=18)")
plot( gb, add=TRUE)
dev.off()
