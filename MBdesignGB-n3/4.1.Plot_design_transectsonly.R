...
newSites <- readOGR( dsn="~/MBHdesignGB/outputs/Design2_Transects_2020-05-15.shp")
zones <- readRDS( "~/MBHdesignGB/outputs/GBZones_forDesign1.RDS")
GB_rasters <- readRDS( "~/MBHdesignGB/outputs/GBRasters_forDesign1.RDS")
inclProbs <- raster( "~/MBHdesignGB/outputs/inclProbs_design1.tif")
...
pdf( "C:/Users/21933549/Dropbox/UWA/Research Associate/Ningaloo19_Data/plots/HistroicBRUVS1.pdf", height=7, width=8)
plot( zones[["AMP"]], main="Historic Samples", col=grey(0.75), border=grey(0.75))
plot( zones[["IUCN2"]], col=grey(0.75), border='cyan', add=TRUE)
plot( zones[["northControl"]], col=grey(0.75), border='blue', add=TRUE)
plot( zones[["southControl"]], col=grey(0.75), border='blue', add=TRUE)
tmp <- as.numeric( BRUVS@data$year)
plot( BRUVS, add=TRUE, col=tmp)
plot( coastLine, add=TRUE, col='red')
legend( "topleft", legend=c("2006", "2009","2013","2014","2015"), bty='n', col=c(1:6)[-3], pch=3)#c(2006, 2009, 2013, 2014, 2015), pch=3)
dev.off()

pdf( "~/MBHdesignGB/outputs/SampleDesign_zones2.pdf")
...
muzsites <- raster::intersect(newSites, zoney)
...
tmp <- crop( tmp, zoney)
plot( tmp, add=TRUE)
spzsites <- raster::intersect(newSites, zoney)
...
tmp <- crop( tmp, zoney)
plot( tmp, add=TRUE)
hpzsites <- raster::intersect(newSites, zoney)
...
#plot( newSites[as.vector( !is.na( zoneID)),], add=TRUE, col='black', pch=20)
#legend( "topleft", legend=c("Legacy Sites","New Sites"), bty='n', col=c("red","blue"), pch=c(4,20))
npzsites <- raster::intersect(newSites, zoney)
...
pdf( "~/MBHdesignGB/outputs/SampleDesign_total2.pdf", height=7, width=8)
plot( gb_rasters$bathy, main = "Geographe Bay 1km, not-clustered transects")
