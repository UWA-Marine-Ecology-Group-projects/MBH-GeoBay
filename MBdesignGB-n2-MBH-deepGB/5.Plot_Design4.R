...
newSites <- readOGR( dsn="~/MBdesignGB/Design4_notClustTransects_2020-05-17.shp")
#zones <- readRDS( "~/MBHdesignGB/outputs/GBZones_forDesign1.RDS")
GB_rasters <- readRDS( "~/MBdesignGB/GBRasters_forDesign4.RDS")
inclProbs <- raster( "~/MBdesignGB/inclProbs_design4.tif")
...
pdf( "~/MBdesignGB/GeoBay_Area.pdf", height=6, width=10)
...
pdf( "~/MBH/SampleDesignNPZHPZ_TransectsNotClus3_bathy.pdf")
...
tmp <- crop( tmp, zoney)
plot( tmp, add=TRUE)
spzsites <- raster::intersect(ns, zoney)
plot( ns, add=TRUE, col='black', pch=20)
#zoneID <- over( legacySites, zoney)
#plot( legacySites[as.vector( !is.na( zoneID)),], add=TRUE, col='red', pch=4)
#zoneID <- over( newSites, zoney)
#plot( newSites[as.vector( !is.na( zoneID)),], add=TRUE, col='black', pch=20)
...
tmp <- crop( tmp, zoney)
plot( tmp, add=TRUE)
hs <- raster::intersect(ns, zoney)
plot( hs, add=TRUE, col='black', pch=20)
#zoneID <- over( legacySites, zoney)
#plot( legacySites[as.vector( !is.na( zoneID)),], add=TRUE, col='red', pch=4)
#zoneID <- over( newSites, zoney)
#plot( newSites[as.vector( !is.na( zoneID)),], add=TRUE, col='black', pch=20)
...
#plot( newSites[as.vector( !is.na( zoneID)),], add=TRUE, col='black', pch=20)
#legend( "topleft", legend=c("Legacy Sites","New Sites"), bty='n', col=c("red","blue"), pch=c(4,20))
npzsites <- raster::intersect(ns, zoney)
plot( ns, add=TRUE, col='black', pch=20)
...
pdf( "~/MBdesignGB/SampleDesign_notClustTransects4.pdf", height=7, width=8)
