...
s.dir <- "~/MBdesignGB/SpatialData/"

b1 <- raster("~/MBdesignGB/SpatialData/Geog_CMR_250mBathy.tif")
b2 <- raster("~/MBdesignGB/SpatialData/GBmultib_lidarUTM_CMR.tif")

b3 <- projectRaster(b1, crs="+proj=utm +zone=50 +south +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0")

b2.2 <- raster::extend(b2, b3)
plot(b2.2)

b3.2 <- raster::extend(b3, b2.2)


b5 <- raster::crop(b2.2, b3.2)
plot(b5)
b6 <- raster::crop(b3.2, b2.2)
plot(b6)

extent(b5)
extent(b6)

b7 <- extend(b5, extent(b6))
plot(b7)

b8 <- raster::aggregate(b7, fact=c(25, 25))
plot(b8)
extent(b8)
extent(b6)
b8 <- raster::extend(b8, extent(b6))
...
r <- b8 > -Inf
plot(r)
poly1 <- rasterToPolygons(r)
plot(poly1)

b4 <- raster::mask(b6, poly1, inverse=T)
plot(b4)
bx <- b4

values(bx)[values(bx) > -20] = NA
plot(bx)

writeRaster(bx, paste(s.dir, "GB250mBathyDeep.tif", sep='/'), overwrite=T)
...
gb <- readOGR( dsn="~/MBdesignGB/SpatialData/GeoBay_CMR_UTM.shp")
gb
HPZ <- gb[1,]
SPZ <- gb[2,]
MUZ <- gb[4,]
...
gb_rasters$bathy <- raster( x="~/MBdesignGB/SpatialData/GB250mBathyDeep.tif")
bg_rasters$bathy <-
...
colnames( GBDat) <- c("Eastern", "Northing", "MUZ", "SPZ", "HPZ", "BATHY", "SLOPE")

setwd("~/MBdesignGB/")
saveRDS( GBDat, file="GBData_forDesign4.RDS")
saveRDS( gb_rasters, file="GBRasters_forDesign4.RDS")
saveRDS( zones, file="GBZones_forDesign4.RDS")
